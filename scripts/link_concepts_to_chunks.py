...
def find_evidence_chunk(node_name, node_pages, chunks_by_page, chunks):
    ...
    for p in node_pages:
        if p in chunks_by_page:
            return chunks_by_page[p][0]["chunk_id"]

    all_pages = sorted(chunks_by_page.keys())
    target = node_pages[0]
    nearest = min(all_pages, key=lambda p: abs(p - target))
    return chunks_by_page[nearest][0]["chunk_id"]
...
def main():
    ...
    with driver.session() as session:
        ...
        session.run(
            "MERGE (d:Document {name: $name}) SET d.source_markdown = $src",
            name=DOCUMENT_NAME, src="markdown/sapporo_jido_fuyo_teate_manual.md",
        )
        ...
        session.run("""
            ...
            MERGE (ch)-[:PART_OF]->(d)
        """, chunks=chunks, doc_name=DOCUMENT_NAME)
        ...
        for n in nodes:
            ...
            chunk_id = find_evidence_chunk(n["name"], node_pages, chunks_by_page, chunks)
